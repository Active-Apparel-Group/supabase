...
`tracking.tracking_timeline_template_item` also includes:

| Column | Type | Default | Purpose |
|--------|------|---------|---------|
| `duration_value` | `integer` | null | Default task duration for this milestone type |
| `duration_unit` | `offset_unit_enum` | 'DAYS' | Default unit for duration (DAYS or BUSINESS_DAYS) |

**Key Concept**: Templates define the DEFAULT duration for each milestone type. When a plan is created from a template, these defaults are copied to the timeline entries, where they can be overridden per instance.
...
// Save template item with duration
async function saveTemplateItem(data: MilestoneFormData): Promise<void> {
  const { error } = await supabase
    ...
    .update({
      ...
      depends_on_template_item_id: data.depends_on,
      offset_value: data.offset_value,
      offset_unit: data.offset_unit,
...
- `offset_value` = Gap AFTER predecessor completes (e.g., 2 days after Proto Ex-Factory)
- `duration_value` = How long THIS task takes (e.g., Fit Comments review takes 7 days)
